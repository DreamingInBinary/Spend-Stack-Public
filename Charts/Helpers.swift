...
public class ChartData: ObservableObject, Identifiable {
    ...
    public func lowestPoint() -> Double {
        return self.points.map{ $0.1 }.sorted(by: { $0 < $1 }).first ?? 0.0
    }
    
    public func highestPoint() -> Double {
        return self.points.map{ $0.1 }.sorted(by: { $0 > $1 }).first ?? 0.0
    }
    
    public func sumOfPoints() -> Double {
        return self.points.map{ $0.1 }.reduce(0, { x, y in
            x + y
        })
    }
}
...
public class MultiLineChartData: ChartData {
    ...
    public init<N: BinaryFloatingPoint>(points:[N], gradient: GradientColor, fill: Color = Color("Primary")) {
        self.gradient = gradient
        self.fill = fill
        super.init(points: points)
    }
    
    public init<N: BinaryFloatingPoint>(points:[N], color: Color, fill: Color = Color("Primary")) {
