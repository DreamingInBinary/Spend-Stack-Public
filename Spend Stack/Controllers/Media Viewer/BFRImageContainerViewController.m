...
#import "SSImageContainerViewController.h"
#import "SSBackLoadedImageSource.h"
...
@interface SSImageContainerViewController () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation SSImageContainerViewController
...
- (void)viewDidLoad {
    ...
    if ([self.imgSrc isKindOfClass:[NSURL class]]) {
        ...
        [self.view addSubview:self.progressView];
        [self retrieveImageFromURL];
    } else if ([self.imgSrc isKindOfClass:[SSBackLoadedImageSource class]]) {
        self.assetType = BFRImageAssetTypeRemoteImage;
        ...
        self.imgLoaded = ((SSBackLoadedImageSource *)self.imgSrc).image;
        [self addImageToScrollView];
    } else {
