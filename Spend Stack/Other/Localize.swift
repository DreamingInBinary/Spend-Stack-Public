...
/*
 You can enable or disable the the potentially untranslated warning
 */
let enableUntranslatedWarning = false

let enableUnusedKeyError = false


/*
 Put your path here, example ->  Resources/Localizations/Languages
 */
let relativeLocalizableFolders = "/Spend Stack/Supporting Files/"
 ...
 */
let relativeSourceFolder = "/Spend Stack/"
...
let unused = enableUnusedKeyError ? masterKeys.subtracting(usedKeys).subtracting(ignored) : []
...
for file in localizationFiles {
    for k in masterLocalizationfile.keyValue.keys {
        if let v = file.keyValue[k] {
            if v == masterLocalizationfile.keyValue[k] && enableUntranslatedWarning {
